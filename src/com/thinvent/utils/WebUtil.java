...
public class WebUtil {
    ...
    public static void write(HttpServletResponse response, String data) {
        try {
            // 设置响应头
            response.setContentType("application/json; charset=utf-8");     // 指定内容类型为 JSON 格式
    ...
    public static void writeJSON(HttpServletResponse response, Object data) {
        try {
            ...
            // 向响应中写入数据
            PrintWriter writer = response.getWriter();
            writer.write(GsonUtil.bean2Json(data)); // 转为 JSON 字符串
            writer.flush();
            writer.close();
        } catch (Exception e) {
            logger.error("在响应中写数据出错！", e);
    ...
    public static void writeHTML(HttpServletResponse response, Object data) {
        try {
            // 设置响应头
            response.setContentType("text/html; charset=utf-8"); // 指定内容类型为 HTML 格式
            response.setCharacterEncoding("UTF-8"); // 防止中文乱码
            // 向响应中写入数据
            PrintWriter writer = response.getWriter();
            writer.write(GsonUtil.bean2Json(data)); // 转为 JSON 字符串
    ...
    public static Map<String, String> getRequestParamMap(HttpServletRequest request) {
        ...
        try {
            ...
            if (method.equalsIgnoreCase("put") || method.equalsIgnoreCase("delete")) {
                String queryString = URLDecoder.decode(IOUtils.toString(request.getInputStream()), "UTF-8");
        ...
        try {
            ...
            if (cookieArray != null) {
                for (Cookie cookie : cookieArray) {
                    if (Strings.isNullOrEmpty(name) && name.equals(cookie.getName())) {
