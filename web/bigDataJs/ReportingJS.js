...
function GetReportingList(strPageIndex,pageSize) {
   $.ajax({
       type: "post",
       url: projectLocation
		+ "servlet/ThirdReport?method=list&pageIndex="+ strPageIndex+"&pageSize="+pageSize,
...
function GetReportingEntry(data, idx){
    var str = "<tr>";
    str += "<td>" + idx + "<input type='hidden' name='hiddenId' value='"+ data.id +"' /></td>";
    str += "<td><a href='" + data.url + "' target='_blank' >" + data.title + "</a></td>";
    str += "<td>" + data.infoType + "</td>";
    str += "<td>"+data.siteName+"</td>";
    str += "<td>" + data.creatTime.substring(0, 19)+ "</td>";
    str += "<td>" + data.getherTime+ "</td>";
    str += "</tr>";
    return str;
}
...
function GetAcquisitionList(strPageIndex,pageSize) {
	$.ajax({
			type : "post",
			url : projectLocation
				+ "servlet/CollectorCfg?method=listInternal&pageIndex="
			...
			success : function(r) {
				...
				if (r == "false") {
					$("#acquisitionList").append("<tr class='noDataSrc'><td colspan='8' style='text-align:center;'>无数据！</td></tr>");
					return false;
				}
				else if(r =="sessionOut"){
	            	doLogout();
...
function SaveAcquisition() {
    ...
    $.ajax({
		type: "post",
		url: projectLocation
			+ "servlet/CollectorCfg?method=editOrAddInternal&id=" + $("#hid_acquisitionId").val()
...
function ExecAcquisition(id) {
        $.ajax({
            type: "post",
            url: projectLocation
					+ "servlet/CollectorCfg?method=excuteInternal&id=" + id,
            dataType: "text",
            async: false,
            success: function (r) {
                if (r == "success") {
