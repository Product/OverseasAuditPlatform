...
window.onload = function () {
    ...
    $("#bt_login").click(function () {
        ...
        $.ajax({
            type: "post",
            url: projectLocation + "servlet/userServlet?methodName=checkuser&userCode="
        					+ strUserCode + "&userPwd=" + strUserPwd,
            dataType: "text",
            async: false,
            success: function (r) {
                if (r != "false") {
                    ...
                    for (var i = 0; i < data.menuList.length; i++) {
                        if (data.menuList[i].Level == "1") {
                            $("#headerMenu").append("<li><a onclick='toTopUrl(this," + i + ")'>" + data.menuList[i].Name + "</a></li>");
                            if (pFlag == 1) {
                                pFlag = 2;
