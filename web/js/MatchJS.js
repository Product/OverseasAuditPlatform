...
function GetNewsFocusList(strPageIndex) {
    ...
    $.ajax({
        ...
        success: function (r) {
            $("#NewsFocusList").html('');
            if (r == "false") {
                $("#NewsFocusList").append("<tr class='noDataSrc'><td colspan='7' style='text-align:center;'>无数据！</td></tr>");
                return false;
            }
            ...
            success: function (data) {
                if (data != "false") {
                    ...
                    for (var i = 0; i < r.Keys.KeysName.length; i++) {
                        ...
                        for(var j = 0; j < r.Keys.KeysContent[i].length; j++)
                            str += "<input class='btn btn-default btn-xs' type='button' class='NewsFocueKeysChioce' value='"+r.Keys.KeysContent[i][j]+"' onclick='addNewsFocusEventKey(this, "+r.Keys.KeysId[i][j]+")'/>";
                        str += "<br>";
                    }
                    $("#hid_NewsFocusDetailParam").val("id=" + Id);
                    $("#NewsFocusDomKeys").html(str);
                    $("#NewsFocusDomContent").html(r.Article.Content);
                }
                showNewsDocumentDomSmall_addEvent(Id);
            },
            error: function (e) {
                alert(e.responseText);
            }
...
function addNewsFocusEventKey(obj, idx){
    $('#txt_NewsFocusEvent').val($(obj).val());
    showNewsDocumentDomLarge_addEvent();
    $('#NewsFocusEventAdd').show();
    $('#NewsFocusEventKeywordId').val(idx);
}
