...
function GetStandardViewList(strPageIndex){
	...
	$.ajax({
		type: "post",
		url: projectLocation + "/servlet/StandardServlet?methodName=QueryList&strQuery=" + strQuery
				+ "&pageIndex=" + strPageIndex + "&pageSize=10&system=2",
		contentType: "application/json;charset=utf-8",
		async: false,
		success: function (r) {
			if (r == "false" || r == "") {
				$("#ruleMonitor_mac .standardMainTable tbody").append("<tr><td colspan='5' style='text-align:center;'>无数据！</td></tr>");
				return false;
			}
			...
			$("#ruleMonitor_mac .standardMainTable tbody").html(str);
		},
		error: function (e) {
			alert(e.responseText);
		}
...
function saveEditStandard(){
	...
	$.ajax({
		...
		url: projectLocation + "servlet/StandardServlet?methodName=" + methodName + "&id=" + $("#hid_standardEditId").val()
				+ "&name=" + name + "&agency=" + agency + "&updateTime=" + updateTime + "&status=" + status + "&remark=" + remark+"&system=2",
		contentType: "application/json;charset=utf-8",
		async: false,
		success: function (r) {
			if(r == "true"){
