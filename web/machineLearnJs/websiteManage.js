...
function GetWebsiteLList(strPageIndex) {
    ...
    $.ajax({
        type: "post",
        url: projectLocation
				+ "servlet/WebsiteServlet?methodName=QueryOutList&strQuery=" + encode(strQuery)
...
function WebsiteCheck(){
	$("#websiteManage_mac").hide();
	$("#websiteManage_check").show();
	FirstPage_websiteCheck();
}


/*商品管理-网站管理-网站核实*/
function FirstPage_websiteCheck(){
    GetMachineWebsiteCheckList(1,10);
}

function PreviousPage_websiteCheck() {
    var pageIndex = (parseInt($("#websiteMachinePaging .pageIndex").text()) == 1 ? 1
			: parseInt($("#websiteMachinePaging .pageIndex").text()) - 1);
    GetMachineWebsiteCheckList(pageIndex,10);
}

function NextPage_websiteCheck() {
...
function EndPage_websiteCheck() {
	GetMachineWebsiteCheckList(parseInt($("#websiteMachinePaging .pageCount").text()),10);
}
 ...
 * @param pageSize
 */
function GetMachineWebsiteCheckList(strPageIndex,pageSize) {
   ...
   $.ajax({
       type: "post",
       url: projectLocation
		+ "./servlet/WebsiteServlet?methodName=NewQueryOutList&strPageIndex="+ strPageIndex+"&strPageCount="+pageSize
		+"&strQuery="+encode(strKeyWords)+"&strStraTime="+strBeginDate+"&strEndTime"+strEndDate+"&webType="+strConductType,
       contentType: "application/json;charset=utf-8",
       async: false,
       success: function (r) {
           $("#websiteCheckManageList").html('');
           if (r == "false") {
               $("#websiteCheckManageList")
           ...
           for (var i = 0; i < r.websiteList.length; i++) {
           	 str += GetMacWebsiteCheckEntry(r.websiteList[i], i+1);
           }
           $("#websiteCheckManageList").append(str);
       },
       error: function (e) {
           alert(e.responseText);
       }
 ...
 * @returns {String}
 */
function GetMacWebsiteCheckEntry(data, idx){
...
function BatchWebAdd(obj){
	...
	var idList = "";
	$("#websiteCheckManageList .mismatchWebSiteChild").each(function(){
...
function WebsiteselectAll(){
	$("#websiteManage_check .mismatchWebSite").change(function(){
			if($(this).prop("checked")){
				$("#websiteManage_check .mismatchWebSiteChild").prop("checked",true);
			}else{
				$("#websiteManage_check .mismatchWebSiteChild").prop("checked",false);
			}
