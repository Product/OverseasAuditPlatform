...
window.onload = function () {
	...
	/*
	var timerBean=1; 
	if($("#goods_opinion_stream").show()){
	     	setInterval(fresh(timerBean),5000);
	}else{
		clearInterval(fresh(timerBean));  
	}*/
    ...
    $("#bt_login").click(function () {
        ...
        $.ajax({
            ...
            + strUserCode + "&userPwd=" + strUserPwd +"&system=3",
            dataType: "text",
            async: false,
            success: function (r) {
                if (r != "false") {
...
function toUrl(obj) {
    ...
    // 产品管理
    if ($(obj).attr('rel') == 'ProductDefinitionDom') {
        GetStreamProManList(1);
    }
    //舆情管理
    else if ($(obj).attr('rel') == 'public_opinion_stream') {
        GetStreamPubOpiList(1);
    }
    
    //商品管理
    if ($(obj).attr('rel') == 'goods_opinion_stream') {
    	FirstPage_mismatch_product();
    }else{
    	stopfresh();
    }
    
    //配置星环
    if ($(obj).attr('rel') == 'rule_management_stream') {
    	SparkFirstPage();
    }
}
